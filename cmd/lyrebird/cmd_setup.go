...
# StartLimitIntervalSec=0 disables the start-rate limiter. For an unattended
# 24/7 device, permanently entering "failed" after a burst of watchdog/crash
# restarts is worse than continuing to retry; per-stream exponential backoff
# and RestartSec already bound the restart rate.
...
# DeviceAllow does NOT expand shell globs; "/dev/snd/* rw" is silently dropped,
# and with DevicePolicy=closed that denies ALL audio devices (no capture).
# "char-alsa" is the ALSA device group (major 116), which is what we need.
SupplementaryGroups=audio
DeviceAllow=char-alsa rw
DevicePolicy=closed

# Runtime/state directories. systemd creates (and cleans) these before the
# sandbox on every start, so the unit no longer fails after a reboot when /run
# (tmpfs) has been wiped. RuntimeDirectory -> /run/lyrebird (== /var/run/lyrebird),
# StateDirectory -> /var/lib/lyrebird; both are added to the writable set.
